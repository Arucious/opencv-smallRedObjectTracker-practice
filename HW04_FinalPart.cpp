// HW04_FinalPart
// See the HW04 writeup PDF for instructions.
// Thia part combines all the pieces you made in Parts 1, 2, and 3.
//
	...
	while (capture.isOpened())
	{
		...
		else if (drawing.size() > 0)
		{
			...
			for (int F = 0; F<faces.size(); F++)
			{
				...
				translateOutline(drawing, faceCenter - drawingCenter); // centers outline around face
				double ratio = sqrt(faces[F].width * faces[F].height / drawingArea); // ratio to normalize the star-to-face measurements that seems to do the best job
				scaleOutline(drawing, ratio * 1.1); // automatically scales to the face size. 
